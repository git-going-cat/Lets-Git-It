...
- 대상: 기존 진행 구현 정책과 맞춰 `switch/checkout`은 점수와 progress 대상에서 제외
...
- 기존 진행 구현에서 `switch/checkout`은 `POSITION_UPDATE` 전용으로 취급되어 점수와 progress에서 제외된다. 이번 만료 스케줄도 이 정책을 유지한다.
