...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		Member member;
		try {
			member = memberService.findOrCreateOAuthMember(email, Provider.GOOGLE, providerId);
		...
		// 3. memberId를 attributes에 추가 — SuccessHandler에서 Redis 저장 시 사용
		Map<String, Object> augmentedAttributes = new HashMap<>(attributes);
		augmentedAttributes.put("memberId", member.getId().toString());
