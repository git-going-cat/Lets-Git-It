...
export default function EditCharacterModal({
  ...
  const handleHairChange = (id: string) => {
    ...
    if (!nextColors.includes(selected.characterHairColor)) {
      handleSelect('characterHairColor', nextColors[0]);
    }
  };
  ...
  const handleOutfitChange = (id: string) => {
    ...
    if (!nextColors.includes(selected.characterOutfitColor)) {
      handleSelect('characterOutfitColor', nextColors[0]);
    }
  };
...
function CharacterSaveNotice({ type, onConfirm }: CharacterSaveNoticeProps) {
  ...
  return (
    <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/35">
